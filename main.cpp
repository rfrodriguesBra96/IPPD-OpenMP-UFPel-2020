...
void nbody(Particle* d_particles, Particle *output, int n) {
...
int main (int argc, char** argv) { 
	...
	for(int timestep = 1; timestep <= number_of_timesteps; timestep++) {
		nbody(particle_array, particle_array2, n);
	...
	printf("Tempo wtime: %.8f segundos\n", time);
	printf("Tempo omp_get_wtime: %.8f segundos\n", (fim - inicio));
